...
Since 1981, the London Marathon has stood as an iconic annual spectacle that has entranced participants from across the globe. Our extensive dataset covers the period from 1981 to 2023, delving deep into the intricacies of the marathon through two crucial components: "london_marathon" and "winners." The former presents a comprehensive overview of the marathon's yearly dynamics, encompassing vital statistics like number of applicants, accepted participants, starters, finishers, charitable contributions in millions of pounds (£MM), and a diverse array of chosen charities. The latter component, the "winners" dataset precisely details the successful individuals in four competition categories: Men, Women, Wheelchair Men, and Wheelchair Women. It doesn't merely provide the champions' names and nationalities but also uncovers their remarkable completion times.
...
men_women <-ggplot(men_women_counts, aes(x = reorder(Nationality, -Wins), y = Wins, fill = Category)) +
...
wheelchair <- ggplot(wheelchair_counts, aes(x = reorder(Nationality, -Wins), y = Wins, fill = Category)) +
  geom_bar(stat = "identity", position = "dodge") +
...
grid.arrange(men_women,wheelchair, ncol = 2)
...
As outlined in the data overview, Kenya boasts the highest count of winners in both the male and female categories. Furthermore, the number of Kenyan winners surpasses that of the host country, the UK, by 50% or more. Consequently, the available data opens the possibility of examining whether Kenyan runners, being the most successful in terms of the number of victories, also exhibit the fastest overall performance. This analysis may shed light on whether speed contributes to Kenya's prominence as the most successful country in these two competition categories.


**Research Question 2: In what ways has the London Marathon's popularity fluctuated over time??**

Throughout the years, the London Marathon has undergone dynamic popularity shifts, exposing captivating trends. Analyzing Acceptance Rate data from the early '80s to the present across diverse categories unveils an engaging narrative of the marathon's evolving appeal. This report explores these fluctuations, unraveling the complex tale of the event's changing dynamics.
...
**Research Q2** : In what ways has the London Marathon’s popularity fluctuated over time?
...
The London Marathon's acceptance rates have undergone dynamic shifts, revealing intriguing patterns over the years which can be visualized in (Fig.2). 
